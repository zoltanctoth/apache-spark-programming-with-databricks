...
def validate_2_1(self, schema:StructType):
    ...
    suite.test_equals(actual_value=type(schema), 
                      expected_value=StructType, 
                      description="Schema is of type StructType",
                      hint="Found [[ACTUAL_VALUE]]")
    
    suite.test_length(schema.fieldNames(), 2, description="Schema contians two fields", hint="Found [[LEN_ACTUAL_VALUE]]: [[ACTUAL_VALUE]]")
...
def validate_4_1(self, query):
    ...
    suite.test_equals(query.name, "active_users_by_traffic", description="The query name is \"active_users_by_traffic\".")
    suite.test_equals(query.lastProgress["sink"]["description"], "MemorySink", description="The format is \"MemorySink\".", hint="Found [[ACTUAL_VALUE]]")
...
def validate_6_1(self, query):
    ...
    suite.test_false(query.isActive, description="The query has been stopped")
